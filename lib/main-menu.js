...
const getView = function (config) {
  const hotkey = (config && config.hotkey) || {}
  return {
    label: 'View',
    submenu: [
      {
        label: 'Reload',
        accelerator: hotkey.reload || 'CommandOrControl+R',
        click () {
      ...
      },
      {
        ...
        accelerator: hotkey.devTool || (macOS ? 'Command + Alt + I' : 'Control + Shift + I'),
        click () {
      ...
      {
        label: 'Next Note',
        accelerator: hotkey.nextNode || 'CommandOrControl+]',
        click () {
      ...
      {
        label: 'Previous Note',
        accelerator: hotkey.prevNode || 'CommandOrControl+[',
        click () {
      ...
      {
        label: 'Focus Search',
        accelerator: hotkey.focusSearch || 'CommandOrControl + Shift + L',
        click () {
      ...
      },
      {
        ...
        accelerator: hotkey.fullScreen || (macOS ? 'Command + Control + F' : 'F11'),
        click () {
      ...
      },
      {
        ...
        accelerator: hotkey.toggleSideBar || 'CommandOrControl+B',
        click () {
      ...
      {
        label: 'Actual Size',
        accelerator: hotkey.actualSize || 'CommandOrControl+0',
        click () {
      ...
      {
        label: 'Zoom In',
        accelerator: hotkey.zoomIn || 'CommandOrControl+=',
        click () {
      ...
      {
        label: 'Zoom Out',
        accelerator: hotkey.zoomOut || 'CommandOrControl+-',
        click () {
...
const getMenu = (config) => {
  ...
  if (process.platform === 'darwin') {
    return [boost, file, edit, view, window, help]
  } else if (process.platform === 'win32') {
    return [boost, file, view, help]
  } else {
