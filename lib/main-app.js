...
app.on('ready', function () {
  ...
  mainWindow.webContents.executeJavaScript('window.localStorage.getItem("config")').then((config) => {
    const template = require('./main-menu')(config ? JSON.parse(config) : { hotkey: {} })
    const menu = Menu.buildFromTemplate(template)
    const touchBarMenu = require('./touchbar-menu')
